...
> - **两列均在运行中**：从 `ScheduledStops` 计划时刻表读取两列车在该冲突站的到达/发车绝对时刻，计算各自的占用窗 `[到达, 发车]`，两窗重叠（10 秒余量）才报紧急冲突。例如 G7031 在镇江站高速场 6:20 到 6:22、G9425 在 6:40 到 6:42，两个占用窗不重叠，不报冲突；而 C3854 在南京站普速场 1 道 5:45 到 6:05 与另一列车 5:30+ 到 5:50+ 重叠，构成冲突。时刻表不可读时回退为"可能冲突"警告。
...
**TTS 合成与语音来源**：预录音频只覆盖车号字母/数字、方向词和提示音；句式词、缺失字母和所有站名由 TTS 合成。TTS 引擎按以下优先级自动选择，无需用户干预：

| 优先级 | 引擎 | 适用场景 |
|--------|------|----------|
| 1 | 在线百度 TTS（`fanyi.baidu.com/gettts`） | 默认启用，国内可达、无需 API key、音质清晰。启动时用短词探测可达性，不通则自动关闭。预录模式下 TTS 段也优先走在线，保留预录音源拼接车号/数字/提示音。 |
| 2 | Windows OneCore（`Windows.Media.SpeechSynthesis`） | Win10 1809+ 自带，可列出 Huihui/Kangkang/Yaoyao 等 OneCore voice；网吧或精简系统组件缺失时合成会超时失败。 |
| 3 | System.Speech（SAPI5） | 最兜底，通常只能看到 Huihui；精简系统可能无中文 voice，此时静默跳过。 |
...
**语音切换菜单**：窗口顶部"语音"菜单可切换语音来源：

- **预录广播音源（男声）** — 默认。预录音频拼接车号/数字/提示音 + 在线百度 TTS 合成站名/句式
- **在线 · 晓晓 / 云希** — 纯在线 TTS（百度接口），车号/数字/站名/句式全部合成，适合无预录素材或想统一音色的场景
- **TTS · Microsoft Huihui / Kangkang / Yaoyao** — 纯本地 TTS（OneCore），无需联网但依赖系统 voice 组件完整
- **安装更多语音…** — 弹窗指引通过 Windows 设置添加更多中文 voice

选择持久化到 `%LOCALAPPDATA%\RailRouteAssistant\voice.json`，重启记住。

**播报速度**：在线百度 TTS 使用 `spd=6`（偏快，配合车站广播节奏）；本地 TTS（OneCore/System.Speech）通过 SSML 设为正常速度的 **1.2 倍**（`+20%`）。数字读音及首尾提示音继续使用原始预录素材，以避免简单变速造成音调变化或失真。
...
- **版本号**：窗口标题栏显示当前桌面程序版本，例如 `Rail Route 调度助手 v2.6.0`；若标题没有版本号，说明启动的是旧版 EXE
