...
- 地图名匹配内置规则后，全图所有“含‘场’且不含‘站’”或“以‘上行’/‘下行’结尾
  ...
  写成完整名，不再前置母站。
- 已内置两条地图规则：地图名含“郑州东站” → 母站“郑州东站”；地图名含“南京枢纽” →
  母站“南京南站”。
- 地图名读取失败时，内置直接映射兜底：京广场→郑州东站京广场、城际场→郑州东站城际场、
  徐兰场上行/下行→郑州东站徐兰场上行/下行，以及京沪场、宁合宁杭场、宁安宁仙场
  分别前置“南京南站”。纯中文名和“英文内部名 + 中文场名”的混合字段均可匹配。
- 用户可通过配置文件 `%LOCALAPPDATA%\RailRouteAssistant\station_groups.txt` 增加
  或覆盖规则，每行一条：
  - `地图名片段|母站名` —— 按地图名匹配，对全图“XX场”子站点前置母站名
  - `=子站点名|完整名` —— 直接映射，优先级最高（覆盖一切）
...
车次详情在始发终到旁明确标注“12306 当日在线”“路路通离线”或“12306 2022 快照”。
因此像 `G7965` 这类仅能从冻结快照找到历史始发终到、但 12306 当天没有运行计划的
车次，不会再被误解为在线接口已经返回了当日数据。
...
短暂失败会自动重试一次，后台预取失败后退避 10 分钟，用户主动再次打开详情仍可立即重试。
...
- **复合车号**：支持相邻、斜杠及三段以上形式，例如 `G6642G6641`、
  `DJ8598G3401`、`0G1703/G1704`、`0Y2/Y1`、`G3220G3219G1792`。列车实际
  到达当前段的 12306 终点后切换到下一段；同一段的逐站车号发生变化时，实际到达
  换号站后切换（例如 `G1792` 到达芜湖后显示为 `G1789`）。
...
- **上半部分（列车列表区）**：按状态排序，不同车次类型用不同背景色区分；点击列头可按该列升序/降序排序，再次点击同一列头切换方向，第三次点击恢复默认排序（右键菜单也有“恢复默认排序”）
...
列车列表列：`车号 | 始发 | 终到 | km/h | 延误 | 信号 | 状态 | 当前停站 | 前方停站 | 站台 | 到达`

“当前停站”仅在列车确实因 `Station` 停车时显示，例如 `南京站高速场 1道`；运行中、信号机前停车和等待入图时留空。“前方停站/站台”仍表示下一计划停站。

“到达”列显示预计时刻（HH:MM）：运行中列车显示预计到达下一站的时刻（游戏时钟 + 距到达剩余秒数）；停站列车显示预计发车时刻。
