...
当前版本不实现普通传感器自动进路与手动进路的来源区分，因此不再要求为该功能
制作“触发前—建立后—通过后”存档。实时功能也不读取存档。后续人工验收只采集
规范化运行时快照和对应画面，不提交地图、游戏程序集或原始对象转储。
...
- [x] 用合成快照覆盖环路终止/顺序稳定、多可行分支保持歧义
- [x] 用合成快照覆盖多列车/多进路隔离和复杂道岔建立、改向、释放确定性
- [x] 建立 `runtime/v1` localhost TCP 接收、分片帧、重连和会话序号隔离
- [x] 打通 Runtime 快照→Operations→Projector→Web 的随机端口端到端测试
- [x] 完成私有加载器及当前游戏 API 映射的本机静态构建验证
- [ ] 在 2—3 张真实复杂地图上执行只读运行时验收
- [ ] 安装私有加载器并执行实际游戏验证
...
- 50 项测试通过（其中 Operations 16 项、Runtime 9 项、Web 6 项）；
