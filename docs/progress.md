...
- [x] 对 97 份真实存档运行只读语料验证
- [x] 用“南通站1/南通站2”受控存档对照确认分配码与进路目标字段
- [ ] 找到能在单份存档中区分手动/自动进路来源的稳定字段

当前 mapper 支持有真实语料的 `2.3.17`、`2.3.18`、`2.3.22`、`2.3.23`、
`2.3.24`。59 份受支持版本存档全部映射成功；其余 38 份旧版本仍可无损读取，
但会被语义 mapper 明确拒绝。

最新自动验证结果：

- 13 项测试通过（其中 schema mapper 6 项）；
- 59/59 份受支持语料映射成功，0 失败；
- 共映射 2,183 个车站快照、60,090 条活动轨道、687 个当前列车快照和
  28,732 条进路开通观察。
