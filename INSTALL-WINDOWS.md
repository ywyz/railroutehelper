...
2. 双击 `RailRouteAssistantDesktop.exe`。标题栏会显示当前版本号；v2.6.5 的标题应为
   `Rail Route 调度助手 v2.6.5`。如果没有版本号或版本更旧，请关闭程序并确认
...
- 双击列车行可打开车次详情；也可以先选中车次后按 `Enter`，或右键选择
  “查看车次详情（双击）”。
- 主窗口“告警中心”可确认、静音和查看已恢复告警；“时距运行图”选择基准列车后
  显示计划、实际和预测运行线；“会话回放”可把本局保存为新建 JSONL 文件并回放。
- 自动记录文件默认位于 `%LOCALAPPDATA%\RailRouteAssistant\sessions`；手动记录可自行
  选择位置。助手不会覆盖已有会话文件。
...
- 地图车号 `0G2524` 保留原样显示和播报，但始发终到查询使用 `G2524`；`DJ54` 读作“动检五四”，`Y` 读作“游”。
- 列表背景色会忽略地图专用的前导 `0` 和“通”再分类；DJ、Y、纯数字及其他特殊字头不再使用无区分的默认背景。
