...
2. 双击 `RailRouteAssistantDesktop.exe`。标题栏会显示当前版本号；v2.7.1 的标题应为
   `Rail Route 调度助手 v2.7.1`。如果没有版本号或版本更旧，请关闭程序并确认
...
- 双击列车行可打开车次详情；也可以先选中车次后按 `Enter`，或右键选择
  “查看车次详情（双击）”。
- 始发终到旁会标明数据来自“12306 当日在线”“路路通离线”或“12306 2022 快照”；
  历史快照有始发终到但 12306 当天无计划时，详情会明确提示两者的区别。
...
- `0G6642G6641`、`DJ8598G3401`、`0G1703/G1704` 以及 `G3220G3219G1792`
  等复合车次会拆成任意多个运行段。列车到达当前段的 12306 终点后切换到下一段；
  同一运行段的 12306 逐站车号发生变化时，到达该换号站后切换。秦皇岛地图的
  `0Y2/Y1` 仍按 `0Y2`、`0Y1` 两段处理。
