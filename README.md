...
- 语音播报：等待入图接近、停站（含调向提示）、通过、中间站发车前预告和发车自动播报，预录音频拼接 + 站名 TTS 兜底（合成语音以 1.2 倍速播报）
- 车次始发终到查询：按车号精确查询 12306；失败时依次降级到随桌面程序发布的授权路路通表和冻结的 12306 `train_list.js` 快照，自动拆分换向合并车号
