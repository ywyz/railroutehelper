...
- 语音播报：等待入图接近、到站（含早点/正点/晚点与调向提示）、通过、中间站发车前预告和发车自动播报；预录素材优先、TTS 缺词补全，可切换真实 TTS 引擎并调整 1–7 档语速
- 车次始发终到查询：按车号精确查询 12306；失败时依次降级到随桌面程序发布的授权路路通表和冻结的 12306 `train_list.js` 快照；查询时自动移除地图专用“通”和前导 `0`，复合车次在首个停车站后切换到第二段编号
- 特殊车号播报：`DJ8598` 读作“动检八五九八”，`0G2524` 保留前导零读作“零高二五二四”
