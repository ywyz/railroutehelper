...
- 调度会话：将结构化列车状态、告警和用户操作记录为不覆盖已有文件的版本化 JSONL，支持异常尾部恢复、暂停、单帧、时间轴、告警跳转和多倍速回放
- 告警中心：稳定 fingerprint、打开/持续/失联/恢复/复发生命周期，以及确认、静音和恢复提示；断线不会误判告警恢复
- 时距运行图：在桌面程序内选择基准列车生成“车站—时间”走廊图，叠加计划、实际、预测、当前时刻和告警；相对时刻不会被伪造成绝对计划
- Windows 一体包：单文件 EXE 内置 BepInEx，首次运行自动发现或询问游戏目录，安装后自动启动 Steam 游戏

详见 [实时调度助手文档](docs/railroute-assistant.md) 和
[调度会话、告警中心与时距运行图](docs/assistant-sessions.md)。
